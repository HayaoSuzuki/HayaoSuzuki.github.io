# Django の Model バリデーションを追いかける ― choices から Hypothesis まで
...
このエントリでは、素朴な`Choices`モデルから始めて、「`choices`は値を制限してくれる」という誤解を解き、CHECK制約とModelFormで足場を固め、最後にHypothesisによるプロパティベーステストでDjangoのModelバリデーションの挙動まで掘り下げる。
